...
// ─────────────────────────────────────────────
app.get('/', (req, res) => {
    res.send('Task 02 API is running');
});
...
// ─────────────────────────────────────────────
// RESERVE STOCK — THE MOST IMPORTANT FUNCTION
// ─────────────────────────────────────────────
    ...
    try {
        ...
        if (existing.length > 0) {
            ...
            return res.json({
                ...
                note: 'Duplicate request — returning existing order'
            });
        }
        ...
        // Create order with 60 minute expiration (natively calculated by MySQL clock)
        const [orderResult] = await conn.query(
            `INSERT INTO orders (status, total, idempotency_key, expires_at)
             VALUES ('RESERVED', ?, ?, DATE_ADD(NOW(), INTERVAL 60 MINUTE))`,
            [total, idempotencyKey]
        );
        ...
        await conn.commit();
        res.status(201).json({ orderId, total, message: 'Order created, stock reserved for 5 minutes' });
    } catch (err) {
        await conn.rollback();
...
app.post('/api/payments', async (req, res) => {
    ...
    try {
        ...
        if (orders[0].status !== 'RESERVED') {
            throw new Error(`Order is not awaiting payment (status: ${orders[0].status})`);
        }
...
// ─────────────────────────────────────────────
// ORDERS — List all  ⭐ MUST COME BEFORE /:id
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// ORDERS — Get one (with items) + Lazy expiry check
// ─────────────────────────────────────────────
app.get('/api/orders/:id', async (req, res) => {
    try {
        ...
        // Lazy expiry check — natively check via MySQL clock
        const [isExpired] = await db.query(
            "SELECT id FROM orders WHERE id = ? AND status = 'RESERVED' AND expires_at < NOW()",
            [req.params.id]
        );
        if (isExpired.length > 0) {
            ...
            try {
                ...
                await conn.rollback();
                console.error('Lazy expiry error:', err.message);
            } finally {
                conn.release();
            }
        }
...
// ─────────────────────────────────────────────
// AUTO-EXPIRE reserved orders after 5 minutes
// ─────────────────────────────────────────────
setInterval(async () => {
    try {
